...
export class EconomyManager {
	public static async Daily(interaction: ChatInputCommandInteraction, account: Account): Promise<DailyData> {
		const { Clan, DailyStreak, NetWorth, TotalNetWorth, Wallet } = account;
		const cash = 1000;
		...
		const earned = (cash + clanBonus) * streakBonus;

		await UpdateEntity(Account, { Snowflake: interaction.user.id }, { DailyStreak: resetStreak ? 0 : DailyStreak + 1, NetWorth: NetWorth + earned, TotalNetWorth: TotalNetWorth + earned, Wallet: Wallet + earned });

		return { cash, clanBonus, dailyStreak: resetStreak ? 0 : DailyStreak + 1, earned, netWorth: NetWorth + earned, resetStreak, streakBonus, totalNetWorth: TotalNetWorth + earned, wallet: Wallet + earned };
	}
	...
	public static async Work(interaction: ChatInputCommandInteraction, account: Account): Promise<WorkData> {
		const { Clan, Paycheck } = account;
		const cash = RandomNumber(100, 250);
		const clanBonus = Clan === ClanEnum.None ? 0 : RandomNumber(25, 100);
		const overtime = RandomNumber(0, 100) >= 75 ? 2 : 1;
		const earned = (cash + clanBonus) * overtime;

		await UpdateEntity(Account, { Snowflake: interaction.user.id }, { Paycheck: Paycheck + earned });

		return { cash, clanBonus, earned, paycheck: Paycheck + earned, overtime };
	}
}
