...
const modal = new Modal({ id: "EditBalanceCollect" });
...
execute(modal, async (interaction) => {
	...
	if (amount > account.TokenChest) return void (await CreateResponse.InteractionError(interaction, "You entered an amount higher than what you have in your token chest."));
