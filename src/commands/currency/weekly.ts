...
export class WeeklyCommand extends Command {
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		await Cooldown.SetCooldown(`weekly_${interaction.user.id}`, Time.Week, interaction.user.id);

		const weekly = await AccountManager.ClaimWeekly(interaction.user.id);
		const xp = await AccountManager.AddXpToAccount(interaction.user.id, { multiplier: 3 });
		if (account.Clan !== JoinableClan.None) await ClanManager.AddXpToClan(account.Clan);

		await interaction.reply({ embeds: [Utility.CreateSimpleEmbed(`You have claimed your weekly reward of ${EnvData("EMOJI_TOKEN")} **${weekly.tokensEarned.toLocaleString()}**! Come back in 1 week to claim again!`)], ephemeral: false });
